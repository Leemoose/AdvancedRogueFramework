...
class MonsterSpawner():
    def __init__(self, MonsterSpawns):
        logger.debug("Initializing MonsterSpawner")
        ...
        logger.info("Boss monsters: %s", self.bossMonsters)
    ...
    def make_monster_pack(self, depth, branch, params, tier):
        if tier < 2 or tier > 3:
            logger.error("invalid tier for monster pack: %d", tier)
            return []
        ...
        for rare in rareAtDepth:
            if rare.group is not None and rare.group in monsterGroups.keys():
    ...
    def spawnMonsters(self, depth, branch):
        logger.debug("Spawning monsters for depth %d, branch %s", depth, branch)
        if depth > 10:
            depth = 10
        ...
        for tier, tier_count in enumerate(encounters):
            for _ in range(tier_count):
                ...
                if monster is not None and monster != []:
                    monsters.append(monster)

        logger.debug("Spawned %d monster groups for depth %d", len(monsters), depth)
        return monsters
